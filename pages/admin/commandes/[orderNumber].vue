<template>
  <div v-if="order">
    ...
    <div class="flex flex-wrap items-center gap-3 mb-1">
      <NuxtLink to="/admin/commandes" class="text-on-surface-muted hover:text-on-surface" aria-label="Retour aux commandes">
    ...
    <p class="text-sm text-on-surface-muted mb-6 ms-8">
      ...
      <span v-if="order.paymentMethod"> · Paiement : {{ paymentLabel(order.paymentMethod) }}</span>
      <span v-if="order.shippingMethod"> · Livraison : {{ order.shippingMethod }}</span>
    </p>
    ...
    <div v-if="nextStatuses.length" class="bg-surface border border-surface-2 rounded-xl p-4 mb-6 flex flex-wrap items-center gap-3">
      <span class="text-sm text-on-surface-muted me-1">Passer à :</span>
      <button
    ...
    <div class="grid lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-2 space-y-6">
        <section class="bg-surface border border-surface-2 rounded-xl overflow-hidden">
          <h2 class="px-4 pt-4 pb-2 font-semibold">Articles</h2>
          ...
          <div class="border-t border-surface-2 px-4 py-3 text-sm space-y-1.5">
            <div class="flex justify-between text-on-surface-muted"><span>Sous-total</span><span>{{ formatPrice(order.subtotal) }}</span></div>
            <div class="flex justify-between text-on-surface-muted"><span>Livraison</span><span>{{ formatPrice(order.shippingCost) }}</span></div>
            <div v-if="order.discount > 0" class="flex justify-between text-on-surface-muted">
              <span>Remise<template v-if="order.promoCode"> ({{ order.promoCode }})</template></span>
              <span>−{{ formatPrice(order.discount) }}</span>
            </div>
            <div class="flex justify-between font-bold text-base pt-1.5 border-t border-surface-2"><span>Total</span><span>{{ formatPrice(order.total) }}</span></div>
          </div>
        </section>

        <!-- Tracking -->
        <section class="bg-surface border border-surface-2 rounded-xl p-4">
          <h2 class="font-semibold mb-3">Numéro de suivi</h2>
        ...
        <!-- Internal notes -->
        <section class="bg-surface border border-surface-2 rounded-xl p-4">
          <h2 class="font-semibold mb-3">Notes internes</h2>
          <textarea
            v-model="notesInput"
            rows="3"
            maxlength="2000"
      ...
      <div class="space-y-6">
        <section class="bg-surface border border-surface-2 rounded-xl p-4">
          ...
          <div class="text-sm space-y-1">
            ...
            <div class="text-xs text-on-surface-muted pt-1">
              {{ order.customerId ? 'Compte client' : 'Commande invité' }}
            </div>
          </div>
        </section>
        ...
        <section v-if="order.billingAddress && !sameAddresses" class="bg-surface border border-surface-2 rounded-xl p-4">
          <h2 class="font-semibold mb-3">Adresse de facturation</h2>
          <AddressBlock :address="order.billingAddress" />
        </section>

        <section v-if="order.stripePaymentId" class="bg-surface border border-surface-2 rounded-xl p-4">
          <h2 class="font-semibold mb-3">Référence paiement</h2>
  ...
  <div v-else class="p-12 text-center text-on-surface-muted">
    ...
    <template v-else>
      ...
      <NuxtLink to="/admin/commandes" class="text-accent underline mt-2 inline-block">Retour aux commandes</NuxtLink>
    </template>
  </div>
</template>
...
function changeStatus(status: string) {
  if (status === 'cancelled' && !window.confirm(`Annuler la commande ${orderNumber} ?`)) return
  patch({ status }, `Statut mis à jour : ${STATUS_LABELS[status]}`)
}

function saveTracking() {
  patch({ trackingNumber: trackingInput.value }, 'Numéro de suivi enregistré')
}

function saveNotes() {
  patch({ notes: notesInput.value }, 'Notes enregistrées')
}
...
const paymentLabel = (code: string) => PAYMENT_LABELS[code] ?? code
