...
function sourceFiles() {
  // master keeps pages/ and components/ at the root; the rebuild uses app/.
  const roots = ['app', 'pages', 'components', 'layouts', 'composables', 'stores', 'plugins', 'server', 'shared', 'scripts']
  ...
  if (existsSync(sitemap)) {
    ...
    if (block) {
      for (const m of block[1].matchAll(/path:\s*'([^']+)'/g)) {
        ...
        const exists = ['app/pages', 'pages'].some(
          (base) =>
            existsSync(join(ROOT, base, `${clean}.vue`)) ||
            existsSync(join(ROOT, base, clean, 'index.vue'))
        )
        if (!exists) {
          bad++
