...
export default defineEventHandler(async (event) => {
  ...
  const staticPages = ['', '/produits', '/guide', '/comparatif', '/blog', '/faq', '/a-propos', '/contact', '/mentions-legales', '/politique-confidentialite', '/cgv', '/retractation', '/impressum', '/batteries']
  ...
  for (const locale of locales) {
    ...
    for (const a of articles) {
      ...
      const loc = `${baseUrl}${locale.prefix}/blog/${slug}`
      const alternates = locales.map(l =>
        `    <xhtml:link rel="alternate" hreflang="${l.hreflang}" href="${baseUrl}${l.prefix}/blog/${slug}"/>`
      ).join('\n')
